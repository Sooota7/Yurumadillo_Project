...
void PAUSE::Pause_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, FadeObject* fade)
{
	g_pDevice = pDevice;
	g_pContext = pContext;
	...
	ScratchImage	image;
	LoadFromWICFile(L"asset\\texture\\pause.png", WIC_FLAGS_NONE, &metadata, image);
...
void PAUSE::Pause_Finalize()
{
	//テクスチャの解放など
	SAFE_RELEASE(g_Texture);

}
void PAUSE::Pause_Update()
{ 
	//キー入力チェック
	//スタートボタンが押されたらシーンを切り替え
	//フェード処理中はキーを受け付けない
	if (Keyboard_IsKeyDownTrigger(KK_P) && (m_Fade->GetFadeState() == FADE_NONE))
	{
		//フェードアウトさせてシーンを切り替える
