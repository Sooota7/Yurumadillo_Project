...
// 右端アンカー・スクロールしない導火線描画
void DrawSpriteRopeRight(XMFLOAT2 size, XMFLOAT4 col, int bno, int wc, int hc)
{
	g_pDevice = Direct3D_GetDevice();
	g_pContext = Direct3D_GetDeviceContext();
	...
	// 右端アンカー：右端が (0,0)、左端が -size.x
	v[0].position = { -size.x, -size.y / 2, 0.0f };
	v[0].color = col;
	v[0].texCoord = { ux, uy };

	v[1].position = { 0.0f, -size.y / 2, 0.0f };
	v[1].color = col;
	v[1].texCoord = { ux + w, uy };

	v[2].position = { -size.x, size.y / 2, 0.0f };
	v[2].color = col;
	v[2].texCoord = { ux, uy + h };

	v[3].position = { 0.0f, size.y / 2, 0.0f };
	v[3].color = col;
	v[3].texCoord = { ux + w, uy + h };
