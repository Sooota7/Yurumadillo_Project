...
void EffectHit::EffectHit_Initialize()
{
...
void EffectHit::SpawnEffectHit(XMFLOAT3& worldPos, float hit)
{
    for (int i = 0; i < MAX_PARTICLE; i++)
    {
        if (!m_p[i].active)
        {
            ...
            // サイズ：0.20〜0.8
            float s = RandRange(1.5f, 1.8f);
            p->size = XMFLOAT2(s, s);

            p->color = XMFLOAT4(1.0f, 1.0f, 1.0f, 0.8f);
...
void EffectHit::EffectHit_Update()
{
    for (int i = 0; i < MAX_PARTICLE; i++)
    {
        if (!m_p[i].active)
...
void EffectHit::EffectHit_Draw(BillboardManager* mgr)
{
    for (int i = 0; i < MAX_PARTICLE; i++)
    {
        if (!m_p[i].active)
        ...
        Billboard* bb = new Billboard(pos, size, col, m_p[i].bno, wc, hc, BILLBOARD_TEXTURE::HIT);
        mgr->Register(bb);
